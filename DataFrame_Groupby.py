...
sortRatingsField_YoannIsBack = cloneDF(mergeRatings)
sortRatingsField_YoannIsBack = sortRatingsField_YoannIsBack.groupby(['movie_id', 'title'])['rating'].agg(
    COUNT=np.size, myAVG=lambda x: x.sum() / float(x.count())).sort_values('COUNT', ascending=False)
print('My info sorted: \n%s' % sortRatingsField_YoannIsBack[:15])
